...
function processProduct(num1, num2, callback) {
  let productTotal = num1 * num2;
  return callback(productTotal);
}
...
function lowerCaseStrings(strings) {
    let lowerCased = [];
    strings.forEach(indexItem => {
      lowerCased.push(indexItem.toLowerCase());
    })
    return lowerCased;
}
...
function removeApple(stringsArray) {
  let filterApples = [];
  stringsArray.filter(indexItem => {
    if (indexItem != 'apple') {
      filterApples.push(indexItem);
  }
  });
  return filterApples;  
}
...
function stringSmash(strings) {
  let combineWords = [];
  return strings.reduce((totalWords, indexItem) => {
    return totalWords += indexItem;
    });
}
...
function getFullNames(runners) {
  let fullNames = runners.map(indexItem => {
    return `${indexItem.last_name}, ${indexItem.first_name}`
  });
  return fullNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(shirt => 
    shirt.shirt_size === tShirtSize)
}
